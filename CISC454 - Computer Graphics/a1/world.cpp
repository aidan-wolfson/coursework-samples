...

{
  ...
  fleaSpawn = false; // set to true for testing
...

{
  ...
  // See if a flea spawns
  //srand(2918);
  if (randIn01() >= mushrooms.size() / 100 && flea == NULL) { // increased the chance of it spawning so that TA could ensure it works properly
        fleaSpawn = true;
  }
  // spawn the flea
  if (fleaSpawn) {
      flea = new Flea(vec2(randIn01(), WORLD_TOP_ROW));
      fleaSpawn = false;
  }
  // move the flea, make it drop mushrooms, and delete it if it reaches the bottom row 
  if (flea) {
        //move the flea 
        flea->updatePos(elapsedTime);
        
        if (randIn01() > 0.2) {
            // drop a mushroom on column
            int col = rint((flea->pos.x - WORLD_LEFT_EDGE) / COL_SPACING - 1);
            int row = rint((WORLD_TOP_ROW - flea->pos.y) / ROW_SPACING);

            vec2 worldPos(WORLD_LEFT_EDGE + (col + 1) * COL_SPACING, WORLD_TOP_ROW - row * ROW_SPACING);  // (same code as in World constructor)

            // check if a mushroom already exists there
            bool exists = false;
            for (int j = 0; j < mushrooms.size(); j++)
                if (mushrooms[j]->pos == worldPos) {
                    exists = true;
        ...
        // if it reaches the bottom of the play area, delete the flea
        if (flea->pos.y < -0.9) {
            flea = NULL;
        }
  }
  ...
  for (int i=0; i<darts.size(); i++) {
    ...
    if (flea) {
        if ((flea->pos - darts[i]->pos).length() < distanceTravelled) {

                 flea->damage += 1;

                 if (flea->damage >= FLEA_MAX_DAMAGE) { // mushroom is destroyed
    ...
    vec2 toSegment = seg->pos - darts[i]->pos;
	float distAlongLine  = ((toSegment*vec2(0,1))*vec2(0,1)).length();  // [YOUR CODE HERE]
	float distPerpToLine = (toSegment - ((toSegment * vec2(0, 1)) * vec2(0, 1))).length();  // [YOUR CODE HERE]
  ...
  // check if flea eats the player
  if (flea) {
    if ((flea->pos - player->pos).length() < 0.75 * ROW_SPACING) {
...

{
  ...
  for (int i=0; i<mushrooms.size(); i++) {
    ...
    // distance of mushroom projection from 'pos'
    // (mushPos - pos)
    vec2 toMushroom = mushPos - pos; // vector pointing from line origin to mush position
    float distAlongLine  = ((toMushroom * dir)*dir).length(); // [YOUR CODE HERE]
    // distance of the mushroom position from line
    float distPerpToLine = (toMushroom - ((toMushroom * dir) * dir)).length(); // [YOUR CODE HERE]
